...
class Train:
    def __init__(self,root):
        ...
        #Top_Image 
        img_top=Image.open(r"Project_Image\recogniation.jpg")
        img_top=img_top.resize((1410,240),Image.Resampling.LANCZOS)
        self.photoimg_top=ImageTk.PhotoImage(img_top)
        f_lbl=Label(self.root,image=self.photoimg_top)
        f_lbl.place(x=0,y=50,width=1410,height=240)
        #Button 
        b1=Button(self.root,text="TRAIN DATA ",cursor="hand2",command=self.train_classifier,font=("times new roman",30,"bold"),bg="darkblue",fg="white")
